function [ particle_plot, quiver_plot ] = UpdatePlot3D( g, ...
    particle_plot, isQuiver, quiver_plot, isScreenshot, frameNo, pathSC )
%   UPDATEPLOT3D Updates the Plot window, plots Particles and Quiver.
...
%   frameNo: Number of current Frame
    ...
    particle_plot = ...
        ...
        g.particles(1:g.np_cur,3), ones(g.np_cur,1), ...
        colors, '.');
    if isQuiver
        delete(quiver_plot);
        valids = g.v_c.validCells == 'F';
        quiver_plot = quiver3(g.grid_mesh.X(valids), ...
            g.grid_mesh.Y(valids), g.grid_mesh.Z(valids), ...
            g.v_c.X(valids), g.v_c.Y(valids), ... 
            g.v_c.Z(valids), 'color','r', 'AutoScale', 'on');
    end
    axis equal
