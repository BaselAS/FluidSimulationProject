function [ L ] = SystemMatrix2D( g )
%   SYSTEMMATRIX2D Build Equation Matrix for Pressure solving
%   g: Grid
    ...
    function [ L ] = InnerEquations()%Construct Matrix Rows for Inner Cells
        ...
        r = repelem(inner, 5);
        offsets = repmat([0; -1; 1; -g.N_e; g.N_e], ...
            numel(inner), 1);
        neighbors = g.fluid.all_e(r) + offsets;
        c = indices_grid(neighbors);

        v = repmat([4; -1; -1; -1; -1], numel(inner), 1);
