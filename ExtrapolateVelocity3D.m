function [ g ] = ExtrapolateVelocity3D( g )
%   EXTRAPOLATEVELOCITY3D Extrapolate velocity to air cells
%   g: Grid

    %% Init variables
    surface = g.fluid.all_e(g.fluid.types == 'S');%Surface cells indices

    offsets = [-1; 1; -g.N_e; g.N_e; -g.N_e^2; g.N_e^2];%Linear Offsets
    ...
    function [ ext_v ] = Extrapolate( idx )%Get extrapolated velocity for cell
        ...
        ext_v = 1/numel(valids) * sum([g.v_c.X(valids) g.v_c.Y(valids) ...
            g.v_c.Z(valids)], 1);
    end
    ...
    while layer < g.ext_layers && not(isempty(invalids))%Loop until 
        ...
        g.v_c.Y(invalids) = ext_v(:,2);
        g.v_c.Z(invalids) = ext_v(:,3);
        
        g.v_c.validCells(invalids) = 'F';%Mark cells as Valid
