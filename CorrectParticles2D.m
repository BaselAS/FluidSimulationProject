function [ p_dest ] = CorrectParticles2D( g, p_src, p_dest )
%   CORRECTPARTICLES2D Fixes positions of particles who either escaped the
...
%   p_v: Particle Velocities
    ...
    function [ escaped ] = DetectEscapedParticles()
        ...
        escaped{2} = find(p_dest(:,2) < g.xmin);
        if isempty(escaped{2})
            escaped{2} = [];
        end
        escaped{1} = find(p_dest(:,2) > g.xmax);
    ...
    function [ inObs ] = DetectInObstaclesParticles()
        ...
        function idx = checkParticle(idx, ob)
            ...
            p_dest(idx,2) < ob.zmin || ...
            p_dest(idx,2) > ob.zmax )
                idx = [];
            end
        end
    ...
    function [ normals, p_Is ] = ObstacleCollisionPoints(ob, inObs)
        [normals, p_Is] = arrayfun( ...
                    @(idx) ...
                    FindIntersectedFace2D(p_src(idx,:), p_dest(idx,:),...
    ...
    while ( isEscaped || isInObs )
        ...
        elseif isEscaped
            ...
            for side = 1:4
                ...
                [normals, p_Is] = arrayfun( ...
                @(idx) FindIntersectedFace2D(p_src(idx,:), p_dest(idx,:),...
                ...
                bounce = 2*normals.*repmat(dot(dir,normals,2),1,2)-dir;
        ...
        elseif isInObs
            ...
            for i = 1:4
                ...
                bounce = ...
                    2*normals{i}.*repmat(dot(dir,normals{i},2),1,2)-dir;
